...
pred <- predict(M1,se=TRUE,
                newdata=data.frame(TempC=seq(15.0,24.7,0.1), CO2C_log=mean(dat$CO2C_log), SolRad_log=mean(dat$SolRad_log), OutdoorTemp=mean(dat$OutdoorTemp)
                ),interval="prediction")
pred1 <- cbind(pred$fit,pred$fit-1.96*pred$se.fit,pred$fit+1.96*pred$se.fit)
pred <- predict(M1,se=TRUE,
                newdata=data.frame(TempC=mean(dat$TempC), CO2C_log=seq(5.7,8.5,0.1), SolRad_log=mean(dat$SolRad_log), OutdoorTemp=mean(dat$OutdoorTemp)
                ),interval="prediction")
...
pred <- predict(M1,se=TRUE,
                newdata=data.frame(TempC=mean(dat$TempC), CO2C_log=mean(dat$CO2C_log), SolRad_log=mean(dat$SolRad_log), OutdoorTemp=seq(-5.5,24,0.1)
                ),interval="prediction")
...
matplot(seq(-5.5,24,0.1),pred4,type="l",col=c(1,2,2),
...
############################################
matplot(seq(15.0,24.7,0.1),bt(pred1)$y.l,type="l",col=c(1,2,2),
        lty=c(1,2,2),lwd = 2,xlab = "Indoor temperature",
        main=paste("Prediction in real domain"),ylab = "Window opening probability")
grid();rug(dat$TempC)
matplot(seq(5.7,8.5,0.1),bt(pred2)$y.l,type="l",col=c(1,2,2),
        lty=c(1,2,2),lwd = 2,xlab = "CO2 concentration",
        main=paste("Prediction in real domain"),ylab = "Window opening probability")
grid();rug(dat$CO2C_log)
matplot(seq(-0.2,7,0.1),bt(pred3)$y.l,type="l",col=c(1,2,2),
        ...
        main=paste("Prediction in real domain"),ylab = "Window opening probability")
grid();rug(dat$SolRad_log)
matplot(seq(-5.5,24,0.1),bt(pred4)$y.l,type="l",col=c(1,2,2),
        lty=c(1,2,2),lwd = 2,xlab = "Outdoor temperature",
        main=paste("Prediction in real domain"),ylab = "Window opening probability")
grid();rug(dat$OutdoorTemp)
...
############################################
matplot(exp(seq(5.7,8.5,0.1)),pred2,type="l",col=c(1,2,2),
        lty=c(1,2,2),lwd = 2,xlab = "CO2 concentration",
        main=paste("Prediction in transformed domain"),ylab = "Window opening probability")
grid();rug(dat$CO2C)
...
matplot(exp(seq(5.7,8.5,0.1)),bt(pred2)$y.l,type="l",col=c(1,2,2),
        lty=c(1,2,2),lwd = 2,xlab = "CO2 concentration",
        main=paste("Prediction in real domain"),ylab = "Window opening probability")
grid();rug(dat$CO2C)
